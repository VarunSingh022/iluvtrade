...
@pytest.mark.skipif(not ALEMBIC.exists(), reason="alembic is not installed")
def test_every_migration_applies_to_a_database_that_already_has_rows(tmp_path: Path) -> None:
    """Step through the revisions one at a time, with data present throughout.

    This is the test that catches the migration bug an empty-database run cannot.
    ``0004_mfa`` was originally autogenerated as a single ``add_column`` with
    ``nullable=False`` and no default. SQLite cannot add such a column to a
    populated table, and its DDL is **not transactional** — so the first two
    columns were added, the third failed, and the revision was never recorded.
    The database was left in a state no revision described, and every subsequent
    upgrade failed with "duplicate column name".

    Against an empty table it passed. Hence this.
    """

    url = f"sqlite:///{tmp_path}/populated.db"
    database = tmp_path / "populated.db"

    # ``alembic history`` prints "<from> -> <to> (head), message", newest first.
    targets: list[str] = []
    for line in reversed(
        _alembic("history", "-r", "base:head", database_url=url).stdout.splitlines()
    ):
        if " -> " not in line:
            continue
        target = line.split(" -> ")[1].split(",")[0].replace("(head)", "").strip()
        targets.append(target)
    assert len(targets) >= 4, f"expected the full revision list, got {targets}"
    ...
    for target in targets:
        result = _alembic("upgrade", target, database_url=url)
        assert result.returncode == 0, (
            f"revision {target} failed against a populated database:\n{result.stderr}"
        )

        if not seeded:
            # Seed as soon as the tables exist, so every later revision runs
            # against rows rather than an empty schema.
            connection = sqlite3.connect(database)
            organization_id = str(uuid.uuid4())
            connection.execute(
                "INSERT INTO organizations (id, slug, name, created_at, updated_at) "
                "VALUES (?, 'seed', 'Seed', '2026-01-01 00:00:00+00:00', "
                "'2026-01-01 00:00:00+00:00')",
                (organization_id,),
            )
            connection.execute(
                "INSERT INTO users (id, email, password_hash, display_name, status, "
                "live_trading_enabled, created_at, updated_at) VALUES "
                "(?, 'seed@example.com', 'x', 'Seed', 'ACTIVE', 0, "
                "'2026-01-01 00:00:00+00:00', '2026-01-01 00:00:00+00:00')",
                (str(uuid.uuid4()),),
            )
            connection.execute(
                "INSERT INTO audit_events (id, organization_id, created_at, action, "
                "resource_type, outcome, payload_json) VALUES "
                "(?, ?, '2026-01-01 00:00:00+00:00', 'seed.event', 'thing', 'success', '{}')",
    ...
    # Every row survived, and the schema matches the models.
    connection = sqlite3.connect(database)
    assert connection.execute("SELECT COUNT(*) FROM users").fetchone()[0] == 1
    assert connection.execute("SELECT COUNT(*) FROM audit_events").fetchone()[0] == 1
    version = connection.execute("SELECT * FROM alembic_version").fetchone()[0]
    connection.close()

    assert version == targets[-1]
    assert _alembic("check", database_url=url).returncode == 0


@pytest.mark.skipif(not ALEMBIC.exists(), reason="alembic is not installed")
def test_the_mfa_revision_defaults_recovery_codes_for_existing_users(tmp_path: Path) -> None:
    """An existing account must come out of the migration usable, not NULL."""

    import sqlite3
    import uuid

    url = f"sqlite:///{tmp_path}/mfa.db"
    assert _alembic("upgrade", "0003_correlation_id", database_url=url).returncode == 0

    connection = sqlite3.connect(tmp_path / "mfa.db")
    connection.execute(
        "INSERT INTO users (id, email, password_hash, display_name, status, "
        "live_trading_enabled, created_at, updated_at) VALUES "
        "(?, 'before@example.com', 'x', 'Before', 'ACTIVE', 0, "
        "'2026-01-01 00:00:00+00:00', '2026-01-01 00:00:00+00:00')",
        (str(uuid.uuid4()),),
    )
    connection.commit()
    ...
    result = _alembic("upgrade", "0004_mfa", database_url=url)
    assert result.returncode == 0, result.stderr

    connection = sqlite3.connect(tmp_path / "mfa.db")
    row = connection.execute(
        "SELECT mfa_secret, mfa_enabled_at, mfa_recovery_hashes FROM users"
    ).fetchone()
    connection.close()

    assert row[0] is None, "an existing account has no secret"
    assert row[1] is None, "and MFA is off"
    assert row[2] == "[]", "but its recovery list is a usable empty array, not NULL"
